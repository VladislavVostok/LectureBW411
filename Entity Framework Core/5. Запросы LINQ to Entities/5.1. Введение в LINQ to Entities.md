...
Вначале здесь происходит добавление данных, если они отсутствуют. Далее применяются Linq-операторы:
...
- **Any / AnyAsync**: возвращает true, если хотя бы один элемент набора определенному условию, иначе возвращает false.
...
- **Except**: возвращает разность двух наборов, то есть те элементы, которые содержатся только в одном наборе
