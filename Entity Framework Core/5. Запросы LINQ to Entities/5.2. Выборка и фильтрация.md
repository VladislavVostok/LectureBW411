...
Выражение `EF.Functions.Like(p.Name!, "%Tom%"))` означает, что мы ищем строки, где в свойстве Name содержится подстрока "Tom". Поскольку Name представляет nullable-тип, то после названия свойства указывается оператор !.
...
- `[ 0-9 ]`: соответствует одному символу из определенного диапазона

- `[ ^x ]`: соответствует одному символу, который не указан после символа ^
