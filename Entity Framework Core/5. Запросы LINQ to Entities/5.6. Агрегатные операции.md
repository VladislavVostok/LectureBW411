...
Метод All() позволяет проверить, удовлетворяют ли все элементы в базе данных определенному критерию. Например, проверим, все ли пользователи работают в компании Microsoft:
...
В результате будут выполняться выражения SQL наподобие:
...
    double avgAge = db.Users
					    .Where(u=>u.Company!.Name=="Microsoft")
                        .Average(p => p.Age);
